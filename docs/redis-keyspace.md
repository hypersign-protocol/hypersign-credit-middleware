...
| Original amount | `<base>:plans:amount:<scope>` | Hash | `planId -> granted amount` | Persistent |
| Available amount | `<base>:plans:remaining:<scope>` | Hash | `planId -> currently available amount` | Persistent |
| Expiry | `<base>:plans:expires:<scope>` | Hash | `planId -> expiresAt` in epoch ms | Persistent |
| Grant time | `<base>:plans:granted-at:<scope>` | Hash | `planId -> grantedAt` in epoch ms | Persistent |
| Business reference | `<base>:plans:reference:<scope>` | Hash | `planId -> referenceId` | Persistent |
| Plan status | `<base>:plans:status:<scope>` | Hash | `planId -> ACTIVE, DEPLETED, EXPIRED, or REVOKED` | Persistent |
| Critical balance | `<base>:plans:critical-balance:<scope>` | Hash | `planId -> immutable threshold` | Persistent |
| Expiration member | `<base>:plans:expiration-member:<scope>` | Hash | `planId -> JSON member used by the global expiry index` | Persistent |
